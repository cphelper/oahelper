...
import { FaUserCircle, FaCheck, FaSpinner, FaCrown, FaCalendar, FaRupeeSign, FaBookmark, FaHeart, FaStar, FaExclamationCircle, FaTrash, FaExternalLinkAlt } from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  // Function to navigate to question
  const handleViewQuestion = (questionId, companyId) => {
    const encryptedQuestionId = encryptId(questionId);
    const encryptedCompanyId = encryptId(companyId);
    navigate(`/questions/${encryptedQuestionId}?company_id=${encryptedCompanyId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden selection:bg-zinc-800 selection:text-white">
      <DotPattern
        width={32}
        height={32}
        cx={16}
        cy={16}
        cr={1.5}
        className="absolute inset-0 opacity-20 z-0 text-zinc-800"
      />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="inline-flex p-1.5 bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-full">
            {[
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-6 py-2.5 rounded-full transition-all duration-300 text-sm font-medium ${
                  activeTab === tab.id
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-[#0A0A0A] border border-white/10 rounded-[2.5rem] p-8 md:p-10 shadow-2xl backdrop-blur-sm">
            {error && (
            ...
            {activeTab === 'profile' && (
              <form onSubmit={handleUpdateProfile} className="space-y-8">
                ...
                <div>
                  <label className="block text-zinc-400 text-sm font-medium mb-3 ml-1">
                    Display Name
                  </label>
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="bg-zinc-900/50 border border-white/10 text-white rounded-2xl pl-11 pr-4 py-4 w-full focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-white/20 transition-all"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-white text-black hover:bg-zinc-200 disabled:bg-zinc-800 disabled:text-zinc-500 font-medium py-4 px-6 rounded-full transition-all transform active:scale-[0.98] shadow-lg hover:shadow-white/10 flex items-center justify-center space-x-2"
                >
                  {loading ? (
                    <>
                  ...
                  <div className={`p-8 rounded-[2rem] border transition-all duration-500 ${
                    ...
                    <div className="flex items-start justify-between mb-6">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h3 className={`text-xl font-medium ${is_premium ? 'text-white' : 'text-white'}`}>
                    ...
                    {!is_premium && (
                      <div className="mt-6">
                        ...
                        <button
                          ...
                          className="w-full bg-white text-black hover:bg-zinc-200 font-medium py-4 px-6 rounded-full transition-all shadow-lg hover:shadow-white/10"
                        >
                          View Premium Plans
                        </button>
                      </div>
